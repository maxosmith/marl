...
def make_reverb_dataset(
    ...
    if isinstance(table, str):
        ...
        tables = collections.OrderedDict([(name, weight) for name, weight in table.items() if weight > 0.0])
        if len(tables) <= 0:
            raise ValueError(f"No positive weights in input tables {tables}")

    # Normalize weights.
    total_weight = sum(tables.values())
    tables = collections.OrderedDict([(name, weight / total_weight) for name, weight in tables.items()])

    def _make_dataset(unused_idx: tf.Tensor) -> tf.data.Dataset:
        datasets = ()
        for table_name, weight in tables.items():
            max_in_flight_samples = max(1, int(max_in_flight_samples_per_worker * weight))
            dataset = reverb.TrajectoryDataset.from_table_signature(
                server_address=server_address, table=table_name, max_in_flight_samples_per_worker=max_in_flight_samples
            )
            datasets += (dataset,)
        if len(datasets) > 1:
            dataset = tf.data.Dataset.sample_from_datasets(datasets, weights=tables.values())
        else:
            dataset = datasets[0]
    ...
    if num_parallel_calls is not None:
        ...
        # `TrajectoryDataset`s.
        num_datasets_to_interleave = os.cpu_count() if num_parallel_calls == tf.data.AUTOTUNE else num_parallel_calls
