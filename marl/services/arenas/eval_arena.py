...
class EpisodeResult(NamedTuple):
  ...
  def to_logdata(self) -> loggers.LogData:
    ...
    return_data = dict_utils.prefix_keys(
        return_data, "episode_return/player_", delimiter=""
    )
...
class EvalArena(base_arena.BaseArena):
  ...
  def run_evaluation(self, players: Any, step: Optional[int] = None):
    ...
    for scenario in self.scenarios:
      ...
      results = tree.map_structure(
          lambda *args: scenario.aggregate_fn([args]), *results
      )
  ...
  def _run_episode(self, game, players) -> EpisodeResult:
    ...
    player_states = {
        id: player.episode_reset(timesteps[id]) for id, player in players.items()
    }
