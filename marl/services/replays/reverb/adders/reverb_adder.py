...
def spec_like_to_tensor_spec(paths: Iterable[str], spec: specs.ArraySpec):
    """Convert a spec into a TF spec."""
    return tf.TensorSpec.from_spec(spec, name="/".join(str(p) for p in paths))


class ReverbAdder(base_adder.BaseAdder):
    ...
    def add_priority_table(self, table_name: str, priority_fn: Optional[priority.PriorityFn]):
    ...
    @classmethod
    def signature(cls, environment_spec: specs.EnvironmentSpec, extras_spec: specs.TreeSpec = ()):
        ...
        spec_step = worlds.Trajectory(
            ...
            start_of_episode=specs.ArraySpec(shape=(), dtype=bool),
            end_of_episode=specs.ArraySpec(shape=(), dtype=bool),
            extras=extras_spec,
        )
