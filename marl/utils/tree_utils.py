...
def to_numpy(values: _types.Tree) -> _types.Tree:
...
def stack(sequence: Sequence[_types.Tree], safe: bool = False) -> _types.Tree:
...
def unstack(struct: _types.Tree, batch_size: int) -> List[_types.Tree]:
...
def assert_equals(x: _types.Tree, y: _types.Tree) -> None:
...
def flatten_as_dict(x: _types.Tree, delimiter: str = "/") -> Mapping[str, Any]:
    ...
    def _flatten(y: _types.Tree, key_prefix: str) -> Mapping[str, Any]:
        ...
        if isinstance(y, (np.ndarray, jnp.DeviceArray, list)):
