...
_SplitFunction = Callable[[types.Tree], PrefetchingSplit]


def device_put(
    iterable: Iterable[types.Tree],
    device: jax.Device,
...
class PutToDevicesIterable(Iterable[types.Tree]):
  ...
  def __iter__(self) -> Iterator[types.Tree]:
