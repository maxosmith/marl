...
class SimpleNet(hk.Module):
    ...
    def __call__(
        ...
        torso = hk.Sequential([hk.Flatten(), hk.Linear(128), jax.nn.relu, hk.Linear(64), jax.nn.relu])
        hidden = torso(timestep.observation.astype(float))
...
class Agent:
    ...
    def loss(self, params: hk.Params, trajs: Transition) -> jnp.ndarray:
        ...
        _, actions, behavior_logits = jax.tree_util.tree_map(lambda t: t[:-1], trajs)
        ...
        discount = jnp.ones_like(trajs.timestep.reward[1:]) * self._discount
        ...
        rhos = rlax.categorical_importance_sampling_ratios(learner_logits, behavior_logits, actions)
        vtrace_returns = vtrace_td_error_and_advantage(baseline, baseline_tp1, timestep.reward, discount, rhos)
...
def run_actor(
    ...
    for i in range(num_trajectories):
        ...
        for _ in range(unroll_len + int(i == 0)):
            ...
            total_return += state.reward
            if state.step_type == dm_env.StepType.LAST:
                returns.append(total_return)
                returns = returns[-100:]
                logging.log_every_n(logging.INFO, "Episode ended with reward: %s", 5, np.mean(returns))
...
def run(*, trajectories_per_actor, num_actors, unroll_len):
    ...
    net = hk.without_apply_rng(hk.transform(lambda ts: SimpleNet(num_actions)(ts)))
    ...
    ts_with_batch = jax.tree_util.tree_map(lambda t: np.expand_dims(t, 0), sample_ts)
    ...
    for i in range(num_actors):
        ...
        args = (agent, key, current_params, q.put, unroll_len, trajectories_per_actor)
...
def main(_):
    run(trajectories_per_actor=100_000, num_actors=2, unroll_len=20)
